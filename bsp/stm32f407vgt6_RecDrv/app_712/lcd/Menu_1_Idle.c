...
if(JT808Conf_struct.LOAD_STATE==1)
	lcd_bitmap(56,13,&BMP_empty, LCD_MODE_SET);
else if(JT808Conf_struct.LOAD_STATE==2)
	lcd_bitmap(56,13,&BMP_full_0, LCD_MODE_SET);
else if(JT808Conf_struct.LOAD_STATE==3)
	lcd_bitmap(56,13,&BMP_full_1, LCD_MODE_SET);

//电源标志
if(ModuleStatus&0x04)
	lcd_bitmap(68,13,&BMP_Battery, LCD_MODE_SET);
else
...
void  Disp_Idle(void)
{
   ...
   */	   	 
	...
	lcd_bitmap( 86, 0, &img_arrow[check_arrow_index(GPS_direction)], LCD_MODE_SET );
	sprintf( buf_speed, "%03d", Speed_gps/10);
